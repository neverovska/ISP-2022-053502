...
logging.basicConfig(level=logging.INFO, filename="app.log", format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')

uargs = argparse.ArgumentParser(description="Console serializer")
uargs.add_argument(
    'path',
    type=str,
...
def try_get_parser(ext: str):
    parser = None
    try:
        parser = FactoryMethod.create_serializer(ext)
    except ValueError:
